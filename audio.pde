...
FFT trans;
String load_name, save_name, author, title;
...
void setup() {
  ...
  load_name = "./data/gangnam_style.mp3";
  save_name = load_name.substring(0, load_name.length()-4) + "-2.wav";
  //save_name = 
  name_entered = false;
  loaded       = false;
  playing      = false;
...
void draw() {
  ...
  if (name_entered && !loaded) {
    try {
      player = minim.loadFile(load_name, 256);
      save_name = load_name.substring(0, load_name.length()-4) + "-2.wav";
      recorder = minim.createRecorder(player, save_name, false);
      loaded = true;
  ...
  if (loaded) {
    ...
    noFill();
    rect((width/2) - 10, 300, 20, 20);
    stroke(255,0,0);
    fill(255,0,0);
    if(recorder.isRecording()) {
       rect((width/2) - 9, 301, 18, 18);
    }
    stroke(100);
    fill(100);
...
void keyPressed() {
  if (key == '\n' && !loaded) {//pressing enter after typing URL
    name_entered = true;
  } 
  else if (key == BACKSPACE && !loaded) {//handle deletions to URL
    if (load_name.length() > 0)
      load_name = load_name.substring(0, load_name.length() - 1);
  } 
  else if (!name_entered) {//entering URL
    load_name += key;//
  } 
  ...
  else if (key == 's' && loaded && !recorder.isRecording()) {//save audio.  must use type that processing supports
    recorder.save();
  }
  else if (key == 'r' && loaded) {//record samples for saving
    if (recorder.isRecording()) {
      recorder.endRecord();
    } 
    else {
